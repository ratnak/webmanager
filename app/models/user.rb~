class User < ActiveRecord::Base
  ...
  attr_accessible :first_name,:last_name,:email,:password,:password_confirmation,:login,:persistence_token,:login_count,:last_request_at,:cuurent_login_at,:last_login_at,:last_login_ip,:current_login_ip,:role_attributes,:send_email
#  attr_accessor :roles_attributes
  ...
  private
	...
	def create_role_user
     @user = User.create(:first_name => "admin",:last_name => "admin",:login => "admin",:email => "admin@admin.com", :password => "123456",:pasword_confirmation =>"123456")
     Role.create(:title => "administrator", :user_id => @user.id)
